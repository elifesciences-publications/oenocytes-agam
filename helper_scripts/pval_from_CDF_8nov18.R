...
pval_from_CDF = function(matrix_frequencies,matrix_tpms,
  ...
  dis.ac = data.frame(
    ...
    mean_logtpm   = 1/2 * (rowMeans(log10(tac[,cli]),na.rm = T) + rowMeans(log10(tac[,clj]),na.rm = T))
  )
  
  # inter-replicate distributions of diff_edifreq and mean_readcount
  # (SUPPA uses TPM, but read counts are OK bc this is single-base)
      ...
      for (clx.j in 1:length(clx)) {
        if (clx.i < clx.j) {
          dis.ir = rbind(dis.ir,data.frame(
            ...
            mean_logtpm   = 1/2 * (log10(tac[,clx[clx.i]]) + log10(tac[,clx[clx.j]])) # mean TPM count in the event
          )
          )
        }
      }
  ...
  dis.ir.a = dis.ir
  dis.ir.a = dis.ir.a[base::order(-dis.ir.a$mean_logtpm),]
  
  
  # find position of expression-ordered replicates that
  # is closest to expression of query (and its vicinity)
  dis.ac$ref.closest = sapply(dis.ac$mean_logtpm,    function(x) closestVal.fun(dis.ir.a$mean_logtpm,x))
  ...
  # prepare output
  dis.ac             = subset(dis.ac, select=c("event","freq_diff","mean_logtpm","pval"))
  ...
  plotfun = function() {
    plot(y=dis.ac$freq_diff, x=dis.ac$mean_logtpm,
         ...
         xlab="log10(TPM)",ylab="freq_diff",cex=0.6,
  ...
  return(list(result = dis.ac, plot = plotfun))
